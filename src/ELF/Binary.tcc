...
namespace LIEF {
  namespace ELF {
    ...
    Section& Binary::add_section<true>(const Section & section) {
      ...
      SEGMENT_TYPES segment_type = SEGMENT_TYPES::PT_NULL;
      bool section_is_bss = false;
      switch (section.type()) {
      case ELF_SECTION_TYPES::SHT_DYNAMIC:
        {
          if (not has(SEGMENT_TYPES::PT_DYNAMIC)) {
            Segment dynamic_segment;
            dynamic_segment.type(SEGMENT_TYPES::PT_DYNAMIC);
            dynamic_segment.alignment(section.alignment());
            dynamic_segment.add(ELF_SEGMENT_FLAGS::PF_R);
            if (section.has(ELF_SECTION_FLAGS::SHF_WRITE)) {
              dynamic_segment.add(ELF_SEGMENT_FLAGS::PF_W);
            }
            this->add(dynamic_segment);
          }
      ...
      if (this->segments_.size() > 0) {
        for (Segment* segment : this->segments_) {
          ...
          // BSS
          if (not(
            (    section_is_bss and segment->physical_size() == 0) or
            (not section_is_bss and segment->physical_size() != 0)))
            continue;
