...
void Builder::update_dynamic_entries(void) {
  ...
  ios_.seekp(dynamic_section.file_offset());
  vector_iostream dynamic_table(this->should_swap());
  for (const DynamicEntry* entry : this->binary_->dynamic_entries_) {
    Elf_Dyn dynhdr;
    dynhdr.d_tag = static_cast<Elf_Sxword>(entry->tag());
    dynhdr.d_un.d_val = static_cast<Elf_Xword>(entry->value());

    dynamic_table.write_conv<Elf_Dyn>(dynhdr);
  }
  dynamic_section.content(dynamic_table.raw());
}
...
void Builder::build_sections(void) {
  ...
  for (Section& section : this->binary_->sections()) {
    ...
    // We make sure mapped sections are correctly fixed and are contained inside a segment
    if (section.type() != ELF_SECTION_TYPES::SHT_NOBITS and section.name() != ".shstrtab") {
      if (section.file_fixed() != section.memory_fixed()) {
        throw builder_error("Section: '" + section.name() + "' is partially fixed");
      }
      if (section.segments().size() == 0) {
        throw builder_error("Section: '" + section.name() + "' is loadable but no segment is associated");
      }
    }
  ...
  for (Section& section : this->binary_->sections()) {
    switch (section.type()) {
      ...
      case ELF_SECTION_TYPES::SHT_DYNAMIC:
        {
          if (this->binary_->has(SEGMENT_TYPES::PT_DYNAMIC)) {
            auto& dynamic_segment = this->binary_->get(SEGMENT_TYPES::PT_DYNAMIC);
            dynamic_segment.file_offset(section.file_offset());
            dynamic_segment.virtual_address(section.virtual_address());
            dynamic_segment.physical_address(section.virtual_address());
            dynamic_segment.virtual_size(section.size());
            dynamic_segment.physical_size(section.size());
          }
        }
  ...
  // TODO: redesign this
  Elf_Addr got_address = static_cast<Elf_Addr>(-1);
  for (const Relocation& relocation : this->binary_->pltgot_relocations()) {
    if (relocation.has_symbol()) {
      if (got_address > relocation.address()) {
        got_address = relocation.address();
      }
    }
  }


  // TODO: redesign the code so we won't have to write these headers twice
...
void Builder::build_segments(void) {
  ...
  for (Segment* segment : this->binary_->segments_) {
    ...
    if (segment->sections() == std::end(segment->sections())) {
      continue;
    }
    ...
    if (size > segment->physical_size()) {
      LOG(WARNING) << "Segment is too small to contain all sections";
    }
  }
  ...
  for (Segment* segment : this->binary_->segments_) {
      ...
      if (segment->alignment() < 2) {
        continue;
      }
      // Special case for BSS segment
      if (segment->virtual_size() != 0x0 and segment->physical_size() == 0x0) {
        segment->virtual_size(align(segment->virtual_size(), 0x1000));
        continue;
      }
      auto&& content = segment->content();
      size_t aligned_size = align(content.size(), 0x1000);
      if (aligned_size != content.size()) {
        content.resize(aligned_size);
...
void Builder::build_dynamic(void) {
  ...
  if (not this->has_dynamic_symtab) {
    ...
    this->binary_->add(DynamicEntry{ DYNAMIC_TAGS::DT_SYMENT, sizeof(Elf_Sym) });
  }
...
void Builder::build_dynamic_symbols(std::vector<uint8_t>& dynamic_strings_raw) {
  ...
  if (not this->has_dynamic_symtab) {
    ...
    dynsym.alignment(sizeof(Elf_Word));
...
void Builder::build_dynamic_strings(std::vector<uint8_t>& dynamic_strings_raw) {
  ...
  if (this->has_dynamic_strtab) {
    ...
    dyn_strtab_section = &this->binary_->section_from_virtual_address(dyn_strtab_va);
...
void Builder::build_symbol_hash(void) {
  ...
  if (it_hash_section == std::end(this->binary_->sections_)) {
    ...
    hash_section.content(new_hash_table);
    this->binary_->add_section<true>(hash_section);
    this->binary_->add({ DYNAMIC_TAGS::DT_HASH, 0x0 });
    return;
  }
...
void Builder::build_symbol_gnuhash(void) {
  ...
  if (it_gnuhash == std::end(this->binary_->sections_)) {
    ...
    gnu_hash_section.alignment(this->binary_->header().identity_class() == ELF_CLASS::ELFCLASS64 ? 8 : 4);
...
void Builder::build_dynamic_relocations(void) {
  ...
  if (it_dyn_relocation      == std::end(this->binary_->dynamic_entries_) and
    ...
    const auto& dynamic_relocations = this->binary_->dynamic_relocations();
    auto&& it_rela_symbol = std::find_if(
        std::begin(dynamic_relocations),
        std::end(dynamic_relocations),
        [](const Relocation& relocation) -> bool
    {
        if (not relocation.has_symbol()) {
          return false;
        }
        // TODO: This is a hack since the information is not accessible shndx)
        //return (relocation.symbol().information() & 0xffffffff'00000000ull) != 0x00000000'00000000ull;
        return relocation.symbol().name().empty() ? false : true;
    });
    uint64_t rela_count = 0;
    if (it_rela_symbol != std::end(dynamic_relocations)) {
      rela_count = std::distance(std::begin(dynamic_relocations), it_rela_symbol);
    }
    ...
    this->binary_->add({ DYNAMIC_TAGS::DT_RELACOUNT, rela_count });
...
void Builder::build_pltgot_relocations(void) {
  ...
  for (const Relocation& relocation : this->binary_->pltgot_relocations()) {
    ...
    if (relocation.has_symbol()) {
      ...
      idx = static_cast<uint32_t>(std::distance(std::begin(this->binary_->dynamic_symbols_), it_name));
      // TODO: Do we need to check if the relocation is R_XXX_JUMP_SLOT?
      if (got_address > relocation.address()) {
        got_address = relocation.address();
      }
    }
  ...
  if (not this->binary_->has(DYNAMIC_TAGS::DT_PLTGOT) and got_address >= sizeof(Elf_Addr) * 3) {
      // On UNIX:
      // GOT[0] -> offset of .dynamic section
      // GOT[1] -> link_map
      // GOT[2] -> _dl_runtime_resolve
      got_address -= sizeof(Elf_Addr) * 3;
      this->binary_->add({ DYNAMIC_TAGS::DT_PLTGOT, got_address });
  }
  ...
  if (it_pltgot_relocation      == std::end(this->binary_->dynamic_entries_) and
    ...
    //this->binary_->add({ DYNAMIC_TAGS::DT_PLTGOT, 0x0 });
